...
```
```
['2021/01/01', '2021/01/08', '2021/01/15', '2021/01/22', '2021/01/29', '2021/02/05', '2021/02/12', '2021/02/19', '2021/02/26', '2021/03/05', '2021/03/12']
[2767.0, 3105.0, 3310.0, 3097.0, 3203.0, 3060.0, 2992.0, 2885.0, 3041.0, 3268.0, 3272.0]
```
```python
list_by_day(df2, 6)
```
```
['2021/01/02', '2021/01/09', '2021/01/16', '2021/01/23', '2021/01/30', '2021/02/06', '2021/02/13', '2021/02/20', '2021/03/06', '2021/03/13']
[2936.0, 3202.0, 3046.0, 3203.0, 3447.0, 3066.0, 3024.0, 2975.0, 2805.0, 3075.0]
```
```python
list_by_day(df2, 7)
```
```
['2021/01/03', '2021/01/10', '2021/01/17', '2021/01/24', '2021/01/31', '2021/02/07', '2021/02/14', '2021/02/21', '2021/03/07', '2021/03/14']
[3037.0, 2906.0, 3035.0, 3073.0, 2906.0, 2763.0, 2911.0, 3139.0, 2752.0, 2933.0]
```
...
for i in range(2, 10):
    test_lst.append(tmp_str + str(i))
test_lst.append('2021/03/10')
test_lst.append('2021/03/11')
print(test_lst)
```
```
['2021/01/15', '2021/01/16', '2021/01/17', '2021/01/18', '2021/01/19', '2021/01/20', '2021/01/21', '2021/01/22', '2021/01/23', '2021/01/24', '2021/01/25', '2021/01/26', '2021/01/27', '2021/01/28', '2021/01/29', '2021/01/30', '2021/03/02', '2021/03/03', '2021/03/04', '2021/03/05', '2021/03/06', '2021/03/07', '2021/03/08', '2021/03/09', '2021/03/10', '2021/03/11']
```
...
以下舉一個例子，假設預測 03/11 往後的 7 天 (含 03/11)，取往前 13 天的平均，所計算出的 RMSE

```python
print_evaluate('2021/03/11', 7, 13)
```
```
from: 2021/03/11 interval: 7 days
actual data: [3077.0, 3272.0, 3075.0, 2933.0, 3074.0, 3087.0, 3183.0]
pred data:   [3050, 3050, 3050, 3050, 3050, 3050, 3050]
RTMS error: 109.51907857277001
```
...
- 由於星期一的備載容量通常較高，所以將預測的星期一的備轉容量拉高 70
- 由於星期日通常是該周備轉容量最低的，所以將預測的星期日的備轉容量拉低 100
(數字 70、100 是我憑感覺亂選的，也許可以用暴力法挑出最優的數字，但有 overfitting 的疑慮)
...
from: 2021/03/11 interval: 7 days
actual data: [3077.0, 3272.0, 3075.0, 2933.0, 3074.0, 3087.0, 3183.0]
pred data:   [3050, 3050, 3050, 2950, 3120, 3050, 3050]
RTMS error: 101.48961945510909
```
```python
n1, n2 = 7, 15
# test
...
經過上述微調後，3月份預測的 RMSE 降低 6 到 10 左右
...
假設預測 03/11 往後的 7 天 (含 03/11)，取往前 2 周同為周四的平均 (03/04、02/25)，所計算出的 RMSE

```python
print_evaluate('2021/03/11', 7, 2)
```
```
from: 2021/03/11 interval: 7 days
actual data: [3077.0, 3272.0, 3075.0, 2933.0, 3074.0, 3087.0, 3183.0]
pred data:   [3058, 3154, 2890, 2945, 3049, 3134, 3065]
RTMS error: 96.6673234788852
```
```python
n1, n2 = 2, 5
# test
...
就我自己用 `print_evaluate` 反覆觀察後，發現使用這套方法預測三月周一到五的備轉容量會比正確的值還要低，並且考慮到往後天氣有很高的機會越來越熱，所以我將過往**周日以外**的備轉容量全部加上 70 以利於計算上降低誤差，如果加上 70 之後仍小於 3000 ，我會將其值直接設為 3000。 (希望不會 overfitting...)
...
```
```python
print_evaluate('2021/03/11', 7, 4)
```
```
from: 2021/03/11 interval: 7 days
actual data: [3077.0, 3272.0, 3075.0, 2933.0, 3074.0, 3087.0, 3183.0]
pred data:   [3064, 3127, 3068, 2891, 3089, 3165, 3097]
RTMS error: 72.4194133404257
```
```python
n1, n2 = 2, 5
# test
...
date19 = datetime.datetime.strptime('2021/03/19', '%Y/%m/%d')
mar19 = pd.Series({'日期':'2021/03/19', '備轉容量(萬瓩)':327.2, '備轉容量率(%)':10.93, '星期':5, '備轉容量(MW)':3272.0, '日期2':date19})
train_df = train_df.append(mar19, ignore_index=True)

date20 = datetime.datetime.strptime('2021/03/20', '%Y/%m/%d')
mar20 = pd.Series({'日期':'2021/03/20', '備轉容量(萬瓩)':307.5, '備轉容量率(%)':11.48, '星期':6, '備轉容量(MW)':3075, '日期2':date20})
train_df = train_df.append(mar20, ignore_index=True)

date21 = datetime.datetime.strptime('2021/03/21', '%Y/%m/%d')
mar21 = pd.Series({'日期':'2021/03/21', '備轉容量(萬瓩)':298.3, '備轉容量率(%)':11.44, '星期':7, '備轉容量(MW)':2983, '日期2':date21})
train_df = train_df.append(mar21, ignore_index=True)

date22 = datetime.datetime.strptime('2021/03/22', '%Y/%m/%d')
mar22 = pd.Series({'日期':'2021/03/22', '備轉容量(萬瓩)':307.4, '備轉容量率(%)':10.22, '星期':1, '備轉容量(MW)':3074, '日期2':date22})
...
[3188, 3174, 3196, 3340, 3096, 2889, 3156]
```
